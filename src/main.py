...
async def process_payload(request_payload: PayloadBody):
    ...
    income: int = sum(i.penalty for i in optimal_state.contracts) # Note: The penalty is equivalent to the price - Essentially if the job is not taken, the price becomes the penalty.
    path: typing.List[str] = [i.contract_name for i in optimal_state.contracts]
    
    
    # Constructing the response
    response: SuccessfulResponse = SuccessfulResponse(
        income=income,
        path=path
    )
